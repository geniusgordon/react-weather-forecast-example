  ...
  <Container>
    <Col sm="12" md={{ size: 8, offset: 2 }}>
      <Form inline onSubmit={onSubmit}>
        <FormGroup>
          <Input type="select" name="city" id="city" placeholder="City" onChange={onChange} >
            {
              Object.keys(taiwanCities).map(
                (city, key) => <option key={key} value={city}>{taiwanCities[city]}</option>
              )
            } 
          </Input> 
        </FormGroup>
